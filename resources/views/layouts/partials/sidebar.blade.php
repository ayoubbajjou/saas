<div class="container-fluid">
  <div class="row">
    <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
      <ul class="pt-4 mb-auto nav nav-pills flex-column">
        <li class="text-center">
            <img src="{{ Auth::user()->profile_photo_url }}" class="rounded-circle img-thumbnail">
            @if (Laravel\Jetstream\Jetstream::hasTeamFeatures())
              <h5 class="mt-0 mt-2 mb-0 text-center white">{{  Auth::user()->currentTeam->name }}</h5>
            @else
              <h5 class="mt-0 mt-2 mb-0 text-center white">{{ Auth::user()->name }}</h5>
            @endif
            <div class="dropdown">
